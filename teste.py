...
def tratar_feedback(feedback):
    # Expressão regular para encontrar o padrão no feedback
    regex = r'<(\d)(\w)([LD])(\d)(\w)([LD])(\d)(\w)([LD])(\d)(\w)([LD])(\d)(\w)([LD])(\d)(\w)([LD])>'
    
    # Encontrar correspondências usando a expressão regular
    matches = re.findall(regex, feedback)
    
    # Verificar se houve correspondências
    if matches:
        # Extrair as informações de cada grupo
        placa = matches[0][0]
        canal1 = matches[0][1]
        estado1 = matches[0][2]
        canal2 = matches[0][3]
        ...
        estado4 = matches[0][8]
        canal5 = matches[0][9]
        estado5 = matches[0][10]
        canal6 = matches[0][11]
        estado6 = matches[0][12]
        canal7 = matches[0][13]
        estado7 = matches[0][14]
        canal8 = matches[0][15]
        estado8 = matches[0][16]
        
        # Retornar as informações
        return {
            "placa": placa,
            "canal1": {"numero": canal1, "estado": estado1},
            "canal2": {"numero": canal2, "estado": estado2},
            "canal3": {"numero": canal3, "estado": estado3},
            "canal4": {"numero": canal4, "estado": estado4},
            "canal5": {"numero": canal5, "estado": estado5},
...
def on_message(client, userdata, msg):
    ...
    tratar_feedback(feedback)
    resultado = tratar_feedback(feedback)
    print(resultado)
